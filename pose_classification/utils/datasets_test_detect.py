...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        print(type(img))
        return torch.from_numpy(img), labels_out, img_path, (h, w), label_files[index]

    @staticmethod
    def collate_fn(batch):
        img, label, path, hw, label_files = list(zip(*batch))  # transposed
        for i, l in enumerate(label):
            l[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.cat(label, 0), path, hw, label_files[index]
