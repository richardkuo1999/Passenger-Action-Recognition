...
class Net(torch.nn.Module):
    def __init__(self , modelx):
        ...
        self.conv1 = torch.nn.Conv2d(128, 3, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        #self.resnet_layer = torch.nn.Sequential(*list(modelx.children())[6:-1]) can run
        #self.resnet_layer = torch.nn.Sequential(*list(modelx.modules())[31:-1])
        #self.relu = nn.ReLU(inplace=True)
        self.resnet_layer = torch.nn.Sequential(*list(modelx.children())[:-1])
        # resnet50
        self.Linear_layer = torch.nn.Linear(2048, 2)
        # resnet18
        #self.Linear_layer = torch.nn.Linear(512, 2)
...
def pose_resnet18(pretrained=False, progress=True, **kwargs):
    net1 = models.resnet18(pretrained=True)
    #net1 = resnet18()
    output = Net(net1)
    return output

def pose_resnet50(pretrained=False, progress=True, **kwargs):
