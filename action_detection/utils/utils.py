...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.5):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        print(np.shape(pred))
        print('\npred[:,4]:')
        print(np.shape(pred[:,4]))
        print('\npred:')
        print(np.shape(pred[:, 5:].max(1)))
        '''
        ...
        '''
        print('\nclass_conf:')
        print(np.shape(class_conf))
        print('\nclass_pred:')
        print(np.shape(class_pred))
        '''
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None, person_count=0,pose_class=''):
    ...
    if label:
        ...
        #c4 = c2[0] + t_size[0], c2[1] + t_size[1] - 3
        c3 = c2[0] - t_size[1]*10, c2[1] - t_size[1] - 3
        c4 = c2[0] , c2[1] - 3
        cv2.rectangle(img, c3, c4, color, -1)  # filled
        #cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        cv2.putText(img, label, (c3[0], c3[1] + t_size[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
...
def plot_results(start=0, stop=0):  # from utils.utils import *; plot_results()
    ...
    #     'Test Loss']
    s = ['GIoU/XY', 'Acc', 'Confidence', 'Classification', 'Train Loss', 'Precision', 'Recall', 'mAP', 'F1',
         'ResNet Test Loss']
    for f in sorted(glob.glob('results*.txt') + glob.glob('../../Downloads/results*.txt')):
        #results = np.loadtxt(f, usecols=[2, 16, 4, 5, 6, 9, 10, 11, 12, 13]).T
        results = np.loadtxt(f, usecols=[2, 16, 4, 5, 6, 10, 11, 12, 13, 15]).T
