...
class LoadImages:  # for inference
    def __init__(self, path1, path2, img_size=416):
        self.height = img_size

        # Left Cam
        files = []
        if os.path.isdir(path1):
            files = sorted(glob.glob('%s/*.*' % path1))
        elif os.path.isfile(path1):
            files = [path1]
        # Right Cam
        files2 = []
        if os.path.isdir(path2):
            files2 = sorted(glob.glob('%s/*.*' % path2))
        elif os.path.isfile(path2):
            files2 = [path2]

        images = [x for x in files if os.path.splitext(x)[-1].lower() in img_formats]
        images2 = [x for x in files2 if os.path.splitext(x)[-1].lower() in img_formats]
        videos = [x for x in files if os.path.splitext(x)[-1].lower() in vid_formats]
        nI, nV = len(images), len(videos)
        nI2, nV2 = len(images2), len(videos)

        self.files = images + videos
        self.files2 = images2 + videos
        self.nF = nI + nV  # number of files
        self.nF2 = nI2 + nV2  # number of files2
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            img02 = cv2.imread(path2)  # BGR
            assert img01 is not None, 'File Not Found ' + path
            assert img02 is not None, 'File Not Found ' + path2
            print('image1 %g/%g %s: ' % (self.count, self.nF, path), end='')
            print('image2 %g/%g %s: ' % (self.count, self.nF2, path2), end='')

        # Padded resize
        img, *_ = letterbox(img01, new_shape=self.height)
        img2, *_ = letterbox(img02, new_shape=self.height)
        ...
        img2 = img2[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB
        img2 = np.ascontiguousarray(img2, dtype=np.float32)  # uint8 to float32
        img2 /= 255.0  # 0 - 255 to 0.0 - 1.0

        return path, path2, img, img01, img2, img02, self.cap
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False):
        ...
        # Define labels
        #print('img-files'+str(os.path.splitext(self.img_files[0])))
