...
def cal_iou2(xyxy1,xyxy2): #xyxy: cls, x1, y1, x2, y2
    xyxy1 = xyxy1.clone().cpu().numpy()
    xyxy2 = xyxy2.clone().cpu().numpy()
    ixmin = np.maximum(xyxy1[0], xyxy2[0])
    iymin = np.maximum(xyxy1[1], xyxy2[1])
    ixmax = np.minimum(xyxy1[2], xyxy2[2])
    iymax = np.minimum(xyxy1[3], xyxy2[3])
    iw = np.maximum(ixmax - ixmin + 1., 0.)
    ih = np.maximum(iymax - iymin + 1., 0.)
    inters = iw * ih
    # union
    uni = ((xyxy2[2] - xyxy2[0] + 1.) * (xyxy2[3] - xyxy2[1] + 1.) +
           (xyxy1[2] - xyxy1[0] + 1.) *
           (xyxy1[3] - xyxy1[1] + 1.) - inters) 
    overlaps = inters / uni
    return overlaps
